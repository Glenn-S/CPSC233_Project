...
public class GameLoop {
    ...
    public GameLoop() {
        ...
        this.gate =  new ArrayList<Sprite>() ;
     ...
     */
    public GameLoop(ArrayList<Sprite> terrain, ArrayList<Sprite> items,ArrayList<Sprite> gate,ArrayList<Sprite> empty,
        ...
        this.gate = (gate != null)? gate: new ArrayList<Sprite>();
     ...
     */
    public void refreshPrintArray(Player player) {
        int rowTemp;
        int columnTemp;
        ...
        if (player.getKeyCount() < 3) {
        	for (int index = 0; index < this.gate.size(); index++) {
                ...
                columnTemp = this.gate.get(index).getCoord().getxCoord();
                if (this.gate.get(index).getSpriteChar() != ' ') {
                    this.printArray[rowTemp][columnTemp] = this.gate.get(index).getSpriteChar();
                }
        	}
        }
    }
    ...
    public boolean checkCollisions(Player player, String move) {
    	...
    	if (checkEdges(player, move) || checkEnemies(player, move, this.enemy)
                || checkSprites(player, move, this.terrain) || checkSprites(player, move, this.items) || checkSprites(player,move,this.gate)) {
            return true;
        } else {
            return false;
        }
    ...
    public static void main(String[] args) {
        ...
        GameLoop gl2 = new GameLoop(new ArrayList<Sprite>(), new ArrayList<Sprite>(), new ArrayList<Sprite>(), new ArrayList<Sprite>(),
